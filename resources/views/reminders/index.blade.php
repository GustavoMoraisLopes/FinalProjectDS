                ...
                @if($notifications->count() > 0)
                    <div class="list-group list-group-flush">
                        @foreach($notifications as $notification)
                            ...
                            @endphp
                            <div class="list-group-item list-group-item-action {{ $notification->read_at ? '' : 'bg-light' }} d-flex align-items-start gap-3 py-3 notification-item" data-notification-id="{{ $notification->id }}" data-locked="{{ $isLocked ? 'true' : 'false' }}">
                                <div class="form-check mt-2 checkbox-container" style="display: none;">
                                    <input class="form-check-input notification-checkbox" type="checkbox" value="{{ $notification->id }}" id="notif-{{ $notification->id }}" @if($isLocked) disabled title="Este lembrete só pode ser removido após a devolução." @endif>
                                </div>
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="d-flex gap-2 action-buttons">
                                        ...
                                        @endif
                                        @if(!$notification->read_at)
                                            <form action="{{ route('reminders.mark-read', $notification->id) }}" method="POST" class="d-inline mark-read-form">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-success action-btn">
                    ...
                    <form id="selectForm" method="POST" action="{{ route('reminders.delete-selected') }}" style="display: none;">
                        @csrf
...
<script>
    ...
    if (toggleDeleteBtn && cancelDeleteBtn && selectForm) {
        ...
        toggleDeleteBtn.addEventListener('click', function() {
            isDeleteMode = true;
            checkboxContainers.forEach(c => c.style.display = 'block');
        ...
        cancelDeleteBtn.addEventListener('click', function() {
            isDeleteMode = false;
            checkboxContainers.forEach(c => c.style.display = 'none');
        ...
        // Submit do form de delete
        if (deleteSelectedBtn) {
            deleteSelectedBtn.addEventListener('click', function(e) {
                e.preventDefault();
                const selectedIds = Array.from(document.querySelectorAll('.notification-checkbox:enabled:checked'))
                    .map(cb => cb.value);
